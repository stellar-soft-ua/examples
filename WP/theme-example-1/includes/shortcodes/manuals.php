...
add_shortcode('cmt_manuals', 'cmt_manual_callback');

function cmt_manual_callback($atts)
{
    $toxonomy = 'manual_category';
    $terms = get_terms($toxonomy,['orderby'=>'slug', 'order' => 'ASC']);
                    ...
                    <?php foreach ($terms as $key => $term):
                        ...
                        <div class="accardion__item <?= $key !== 0 ? 'toggle-accardion' : '' ?>"
                            ...
                            while ($loop->have_posts()) : $loop->the_post();
                                $manual_url = @get_post_meta(get_the_ID(), 'manual_url', true);
                                if (!$manual_url) {
                                    return;
                                }
                                ?>
