...
function product_add_meta_box() {
    remove_meta_box( 'vna_portsdiv', VNA_POST_TYPE, 'side' );
    remove_meta_box( 'vna_frequencydiv', VNA_POST_TYPE, 'side' );
    add_meta_box( 'product_meta_box', __( 'Сharacteristic' ), 'product_meta_callback',
        VNA_POST_TYPE , 'normal', 'high' );
    add_meta_box( 'product_upper_frequency', __( 'Upper Frequency Product Selector' ), 'product_upper_frequency',
        VNA_POST_TYPE , 'normal', 'high' );
    add_meta_box( 'product_links_meta_box', __( 'Button Links' ), 'product_links_meta_callback',
       VNA_POST_TYPE , 'normal',
        'high' );
//    add_meta_box( 'product_video_meta_box', __( 'Embed Vimeo Video' ), 'product_video_meta_callback',
//       VNA_POST_TYPE , 'normal',
//        'high' );
//    add_meta_box( 'product_testimonial_meta_box', __( 'Select Testimonial' ), 'product_testimonial_meta_callback',
//         VNA_POST_TYPE , 'normal', 'high' );
    add_meta_box( 'software_editor_box', __( 'Software' ), 'software_editor_meta_box',
         VNA_POST_TYPE , 'normal', 'high' );
    add_meta_box( 'applications_editor_box', __( 'Applications' ), 'applications_editor_meta_box',
         VNA_POST_TYPE , 'normal', 'low' );
    add_meta_box( 'vna_specification_meta_box', __( 'Application Solutions Specifications' ), 'vna_specification_meta_box',
        VNA_POST_TYPE , 'normal', 'high' );
    add_meta_box( 'reviews_block_meta_box', __( 'Reviews block' ), 'reviews_block_meta_box',
        VNA_POST_TYPE , 'normal', 'high' );
}

add_action( 'add_meta_boxes', 'product_add_meta_box' );

function product_upper_frequency( $post ){
    $frequencies     = get_terms( [
        'taxonomy'   => VNA_UPPER_FREQUENCY,
        'hide_empty' => false,
        'orderby'    => 'id',
        'order'      => 'ASC',
    ] );
    ...
    <br/>
    <br/>
    <label for="product_type" class="product_type"><?php _e( 'Product Type: ', '' ) ?></label></br>
    <?php $select_options = get_post_meta( $post->ID, 'product_type', true ) ?>
    <select name="product_type">
        <option value="">Choose product type</option>
        <option value="Base" <?php selected( $select_options, 'Base' ); ?>>Base</option>
        <option value="DRA" <?php selected( $select_options, 'DRA' ); ?>>DRA</option>
        <option value="Fx" <?php selected( $select_options, 'Fx' ); ?>>Fx</option>
    </select>
    </br>
    </br>
    <label for="upper_frequency" class="upper_frequency"><?php _e( 'Upper frequency', '' ) ?></label></br>
    <select name="upper_frequency">
        <option value=""><?php echo esc_attr( __( 'Select frequency' ) ); ?></option>
        <?php
        $saved_frequency = get_post_meta( $post->ID, 'upper_frequency', true );
        $select_options  = '';
...
function product_links_meta_callback( $post ) {
    wp_nonce_field( basename( __FILE__ ), 'aft_nonce' );
    $aft_stored_meta = get_post_meta( $post->ID );
    ?>
...
function vna_specification_meta_box( $post ) {

    // Use nonce for verification
    wp_nonce_field( plugin_basename( __FILE__ ), 'myplugin_noncename' );

    $field_value = get_post_meta( $post->ID, 'vna_specifications', false );
    wp_editor( $field_value[0], 'vna_specifications' );

}

function product_video_meta_callback( $post ) {
    wp_nonce_field( basename( __FILE__ ), 'aft_nonce' );
    $aft_stored_meta = get_post_meta( $post->ID );
    ?>
    <p>
...
function product_testimonial_meta_callback( $post ) {
    wp_nonce_field( basename( __FILE__ ), 'aft_nonce' );
    $aft_stored_meta = get_post_meta( $post->ID );
    ?>
    <p>
...
function software_editor_meta_box( $post ) {
...
function applications_editor_meta_box( $post ) {
...
function reviews_block_meta_box( $post )
{
    wp_nonce_field( basename( __FILE__ ), 'aft_nonce' );
    $aft_stored_meta = get_post_meta( $post->ID );
    ?>
...
function product_meta_save( $post_id ) {
    ...
    update_post_meta( $post_id, 'upper_frequency', sanitize_text_field( $_POST['upper_frequency'] ) );
    update_post_meta( $post_id, 'vna_specifications', $_POST['vna_specifications'] );
