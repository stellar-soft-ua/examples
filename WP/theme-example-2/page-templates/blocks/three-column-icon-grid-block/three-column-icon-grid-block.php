<?php
   /**
    * Block Name: Three Column Image  Block
    * 
    */
   ?>
<?php 
   $pol_tcb_heading = get_field('pol_tib_heading');
   $pol_tib_background_color = get_field('pol_tib_background_color');    
   $align_class = $block['align'] ? 'align' . $block['align'] : '';  
   ?>
<?php if(!empty($pol_tcb_heading) || !empty($pol_tib_background_color)) { ?>
<section class="three-column-icon-grid-block padding-full-top-bottom  <?php echo $align_class; ?>" <?php if(!empty($pol_tib_background_color)) { ?> style="background-color:<?php echo $pol_tib_background_color;?>" <?php } ?>>
   <div class="container">
         <?php if(!empty($pol_tcb_heading)) { ?>
         <h2 class="heading text-center"> <?php echo $pol_tcb_heading;?></h2> 
         <?php } ?>
      <div class="row text-center">
         <?php 
            // Check rows exists.
             if( have_rows('pol_tib_icon_grid') ):
              while( have_rows('pol_tib_icon_grid') ) : the_row();
                $pol_tcb_icon_image = get_sub_field('pol_tcb_icon_image');
                $pol_tcb_title = get_sub_field('pol_tcb_title');
                $pol_tcb_description = get_sub_field('pol_tcb_description');
                $pol_th_cta_button = get_sub_field('pol_th_cta_button');
            ?>
         <div class="col-lg-4 col-md-12 col-xs-4" >
            <div class="pro_box">
               <?php if (!empty($pol_tcb_icon_image)) { ?>
               <div class="pro_icon">
                  <img src="<?php echo $pol_tcb_icon_image['url'];?>" class="img-fluid" alt="<?php echo $pol_tcb_icon_image['alt'];?>" />
               </div>
               <?php } ?>
               <div class="circle-info three-b ">
                  <?php if(!empty($pol_tcb_title)) { ?>
                  <h2 class="pro_title" style="color:#0a0a0a"><?php echo $pol_tcb_title;?> </h2>
                  <?php } ?>
                  <?php if(!empty($pol_tcb_description)) { ?>
                      <?php echo $pol_tcb_description;?>
                  <?php } ?>
                  <?php 
                  $link = $pol_th_cta_button;
                  if( !empty($link['title'])):
                  $link_url = $link['url'];
                  $link_title = $link['title'];
                  $link_target = $link['target'] ? $link['target'] : '_self';
                  ?>
                  <a href="<?php echo esc_url( $link_url ); ?>" <?php if(!empty($link_target)){ ?> target="<?php echo esc_attr( $link_target ); ?>" <?php } ?> class="reporting-btn default-btn bg-blue" tabindex="-1"><?php echo esc_html( $link_title ); ?>
                  </a>
                  <?php endif; ?>
               </div>
            </div>
         </div>
         <?php 
            endwhile;
            // No value.
            else :
            // Do something...
            endif;
            ?>
      </div>
   </div>
</section>
<?php  } ?>